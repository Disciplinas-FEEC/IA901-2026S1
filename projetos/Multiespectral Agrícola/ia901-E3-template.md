...
## Conclusão
A partir da proposta deste trabalho, investigar o impacto da integração de informações multiespectrais na segmentação semântica de anomalias agrícolas em imagens aéreas, foi possível obter resultados que fornecem evidências favoráveis para a hipótese inicial: a inclusão de bandas e índices espectrais além do RGB melhora o desempenho global do modelo. Em particular, a configuração RGBNVW apresentou o melhor resultado de mIoU (38,36%), indicando que a combinação entre informação radiométrica bruta (NIR) e atributos derivados (NDVI/NDWI) fornece sinais complementares úteis para a discriminação de padrões complexos no campo.

No entanto, a análise por classe também mostrou que os ganhos não ocorrem de forma uniforme. O uso de NIR e dos índices espectrais favoreceu principalmente classes associadas a vigor vegetativo e dinâmica hídrica, enquanto classes menos frequentes ou com geometria mais sutil permaneceram desafiadoras. Além disso, embora a função de perda ponderada tenha trazido melhoria pontual em classes minoritárias, seu efeito global foi limitado e acompanhado de trade-offs em outras categorias. Esse comportamento reforça que o desbalanceamento severo do Agriculture-Vision continua sendo um fator crítico para a generalização considerando o split de validação adotado.

Como principais desafios, destacam-se: (i) dificuldade de manter o equilíbrio entre as classes, (ii) sobreposição de rótulos e bordas ambíguas, e (iii) dificuldade de manter ganhos consistentes entre todas as categorias simultaneamente. As principais lições aprendidas foram: a engenharia de canais de entrada é decisiva para o desempenho, métricas agregadas devem ser analisadas junto ao desempenho por classe, e estratégias de treinamento para bases desbalanceadas precisam combinar mais de uma técnica (amostragem, loss, calibração e ajustes arquiteturais). Em síntese, o projeto demonstra a viabilidade da abordagem multiespectral e estabelece uma base sólida para avanços futuros em robustez e acurácia por classe.
